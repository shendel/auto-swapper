...
const ActionPanel = (props) => {
  ...
  return (
    <div className="flex">
      ...
      {canEdit && (
        <button className={`bg-blue-500 ${(!canMove) ? 'rounded-l-md' : ''} ${(!canDelete && onAddItem == false) ? 'rounded-r-md' : ''} hover:bg-blue-600 ${buttonClass}`} onClick={onEdit}>
          <FaIcon icon="edit" />
        </button>
      )}
      {onAddItem !== false && (
      ...
      {canDelete && (
        <button className={`${(!canMove && !canEdit && onAddItem == false) ? 'rounded-l-md' : ''} rounded-r-md bg-red-500 hover:bg-red-600 ${buttonClass}`} onClick={onDelete}>
          <FaIcon icon="trash" />
        </button>
      )}
    </div>
